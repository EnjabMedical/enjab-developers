---
title: Enjab UI
description: Version history for the Enjab UI design system. Each entry says what changed and the exact action to adopt it.
icon: Paintbrush
---
...
## 2026.07.10a: Skeletons must be accurate, not generic

`Enjab UI` · 10 Jul 2026

**Changed.** Strengthened the loading rule: a `loading.tsx` skeleton (and any loader) must be an **accurate representation of that specific screen, not a generic placeholder**. Mirror the real page's actual structure, the same header (with or without its action), the same layout kind (a card grid for a card grid, this table's columns for a table, this form's fields for a form), and roughly the same element counts, sizes, and positions. Never reuse one generic skeleton across differently-shaped pages, and never fall back to a lone spinner or a vague pulsing block for page navigation. Someone should recognize which page a skeleton belongs to just by looking at it. See [Foundations](/docs/enjab-ui/foundations) and [Dashboard chrome](/docs/enjab-ui/dashboard-chrome).

**Apply.** Audit each route's `loading.tsx`: does it match that page's real shape (header + action, grid vs table vs form, element counts)? Replace any reused or generic skeleton with a page-specific one.

## 2026.06.08a: Tables are sortable by default

`Enjab UI` · 8 Jun 2026

**Changed.** `@enjab-ui/data-table` now sorts. Every data column is sortable by clicking its header: ascending, then descending, then back to the original (addition) order, with an arrow marking the active column and direction. Handles text (locale + numeric aware), numbers, dates, and addition time; blanks sort last; the sort is stable. New rule: every table column that holds data must be sortable (give it a `sortValue`); leave it off only for an actions column. See [Foundations](/docs/enjab-ui/foundations).
