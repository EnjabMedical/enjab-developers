...
When you update your integration, or Enjab Auth releases a new version, verify your tool against the current contract at https://developers.enjab.ae/llms.txt. This page covers the key points to re-check.
...
A signed-out user, a disabled user, or a user whose role changed at Enjab will keep working in your tool if you cache them. Enjab cannot touch your cookie. The only way central sign-out, account disable, and role changes reach you is if you call `/userinfo` every request.
...
Your tool must not have a login button or login page. The moment a signed-out user touches any page, redirect straight to Enjab's `/authorize` endpoint. Enjab handles passwords, two-factor, and the greeting screen.
...
Your tool does not clear the session or call any sign-out method. A "Sign out" button navigates to your `/api/auth/logout` route. Enjab shows a confirmation page first, so an accidental click never logs anyone out. On confirm, Enjab sends the user back to your logout route with `?confirmed=1`, and only then do you clear your tool's session.
...
By default, Enjab does not send roles. Access is binary: if Enjab let the user in, they have access. Do not gate by role unless your tool is explicitly marked "role-aware" by an Enjab admin.
