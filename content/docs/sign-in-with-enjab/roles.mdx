...
Access is binary. An admin gives a user access to your tool via a role or a direct grant. Enjab enforces that at the door. If the user reaches your tool, they are allowed to use it.
...
An Enjab admin flips your tool to "role-aware" (Admin > Tools > your tool > Role-aware). From then on, the `user` object includes `roles` (global keys like `reception`, `doctor`) and `is_super_admin`.
