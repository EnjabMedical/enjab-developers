...
An Enjab admin registers your tool once in Enjab Auth admin, then shares the credentials with your team.
...
Enjab locks the host to your domain and refuses redirects to any other host.
...
By default, access is binary: if Enjab lets the user in, they can use your tool. Only enable roles if you need to gate features by role inside the tool (see [Roles](/docs/sign-in-with-enjab/roles) for details on how to check roles in code).
