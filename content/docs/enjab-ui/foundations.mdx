...
A button must **never change size** on hover, focus, or active/selected state, that shifts the
layout around it. Change only color or background between states, never the box. The common
trap: a state toggles a `border` on or off on an auto-width button, which makes it 1px wider
or narrower per side. Keep a border in every state, using `border border-transparent` where
you don't want a visible one:
...
**Skeletons and loaders must be an accurate representation of that specific screen, not a generic placeholder.** A skeleton is a low-fidelity portrait of the exact page it replaces: the same header (with or without its action), the same layout kind (a card grid for a card grid, this table's columns for a table, this form's fields for a form), and roughly the same element counts, sizes, and positions. Never reuse one generic skeleton across differently-shaped pages, and never fall back to a lone spinner or a vague pulsing block for page navigation. Someone should recognize which page a skeleton belongs to just by looking at it.
...
## Tables (sortable by default)

Build every table from `@enjab-ui/data-table`, never hand-roll one. Tables are **sortable by
default**: every column that holds data must be sortable by clicking its header. Clicking
toggles ascending, then descending, then back to the original order, and an arrow shows the
active column and direction (up for ascending, down for descending, a faint neutral arrow on
the others). Sorting handles every data type: text (alphabetical, locale and numeric aware so
`Item 2` comes before `Item 10`), numbers, dates, and addition time (the unsorted state is the
original insertion order, so "as added" is always reachable).

Give a column a `sortValue` to make it sortable; the cell can still render anything (an avatar,
a status pill, a link), because sorting uses the raw `sortValue`, not the rendered markup.
Blank values always sort to the bottom regardless of direction, and the sort is stable (equal
rows keep their original order). Leave `sortValue` off only for columns with nothing to sort,
such as a trailing actions column.

```tsx
const columns: Column<Patient>[] = [
  { header: "Name", sortValue: (p) => p.name, cell: (p) => <PatientCell patient={p} /> },
  { header: "Visits", sortType: "number", sortValue: (p) => p.visits, cell: (p) => p.visits },
  { header: "Added", sortType: "date", sortValue: (p) => p.createdAt, cell: (p) => fmt(p.createdAt) },
  { header: "", cell: (p) => <RowActions id={p.id} /> }, // no sortValue: not sortable
];
```

`DataTable` is a client component (sorting is interactive). In a Server Component page, fetch
the rows on the server and pass them to a small `"use client"` table component that defines the
columns, rather than passing column render functions across the server/client boundary.
