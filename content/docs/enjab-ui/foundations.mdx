...
For your own content on dashboards: stack grids to one column on small screens (`grid-cols-1 sm:grid-cols-2 lg:grid-cols-4`), shrink page padding (`p-4 sm:p-6`), never set fixed pixel widths that overflow a phone, keep tap targets comfortable. Do NOT build a separate mobile design, just don't break.
